    ...
    <div class="flex-1 flex">
      ...
      <Show when={!id()}>
        <div class="justify-center items-center flex-1 flex text-2xl font-medium text-zinc-700">
          <p class="">Select a pokemon to see its stats</p>
        </div>
      </Show>
    </div>
  );
...
const SideNav = () => {
  ...
  const activeClass = (pokemonID: string) =>
    id() === pokemonID ? "bg-gray-300" : "hover:bg-gray-100";
        ...
        <For each={pokemonsList.data}>
          {(pokemon) => (
            <Link
              ...
              href={`./?id=${pokemon.id}`}
            >
              <span class="flex bg-zinc-500 border relative justify-center items-center overflow-hidden rounded-sm w-9 h-9 flex-shrink-0">
                <img
